...
class Datas(object):
    ...
    @staticmethod
    def convert_to_int(byte_num: bytes, koef: float = 1) -> int:
    ...
    @classmethod
    def convert_to_bin(cls, byte_num: int, koef: float, length: int) -> str:
        '''Метод конвертации байткода (hex) в двоичную систему'''
        int_number = cls.convert_to_int(byte_num, koef)
        result = bin(int_number)[2:].zfill(length)
        return result

    @classmethod
    def unpack_elem(cls, byte_num: bytes, j_elem: dict):
        '''Метод распаковки элемента'''
        name = j_elem['name']
        koef = j_elem['koef']
        type = j_elem['type']
        if type == 'int':
            return cls.convert_to_int(byte_num, koef)
        if type == 'pr':
            return cls.convert_to_bin(byte_num, koef, j_elem['length'])

        raise ValueError(f'Unknown type in json: {name}, {type}, {j_elem["position"]}')


    @staticmethod
    def unpack_group(value, members: list, data: dict):
    ...
    @classmethod
    def unpack_string(cls, byte_str: bytes, j_elems: dict, data: dict) -> None:
        '''
        ...
        if j_elems['ks'] != byte_str[4:6][::-1].hex():
            return
        data['time'].append(
            cls.convert_to_int(byte_str[:4], j_elems['time_koef']) * 0.001
        ...
        for elem in sorted(j_elems['data_info'], key=lambda x: x['position']):
            ...
            value = cls.unpack_elem(byte_num, elem)
            byte_str = byte_str[length:]
            if not elem['group']:
                data[elem['name']].append(value)
            else:
                cls.unpack_group(value, elem['members'], data)
        ...
        with open(pddfilepath, 'rb') as file:
            ...
            while string:
                for elem in j_data['adr']:
                    cls.unpack_string(string, elem, result['adr'][elem['adr_name']])
                string = file.read(42)
