...
class ReportWindow(BaseWidget):
    ...
    def __init__(self, name: str, controller: "ctrl.Control", parent: "view.MainWindow") -> None:
        ...
        self.flight_data: dict = self.controller.get_data().get('FLIGHT_DATA', {})
        self.initUI()
    ...
    def initUI(self) -> None:
        ...
        if self.flight_data and 'plane' in self.flight_data:
            try:
                self.plane_combo_box.setCurrentIndex(
                    planes.index(self.flight_data['plane']))
            except:
    ...
    def get_input_box(self) -> QHBoxLayout:
        ...
        if self.flight_data and 'intervals' in self.flight_data:
            self.intervals_from_txt.setPlainText(
                str(self.flight_data['intervals']))
