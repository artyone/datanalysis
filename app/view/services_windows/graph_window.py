...
from app.controller import get_intervals_from_string
...
class GraphWindow(QMdiSubWindow):
    ...
    def __init__(self,
        ...
        Args:
            data (pd.DataFrame): данные для графика 
            tree_selected (list): выбранные столбцы
            decimation (int): прореживание
            start_time (int): время начала построения графика
            stop_time (int): время конца построения графика
            parent (view.MainWindow): родительское окно
        '''
        super().__init__()
        self.parent = parent
        self.data: dict = data
    ...
    def create_graph(self) -> None:
        ...
        self.plot.showGrid(x=True, y=True)  # показывать сетку
        ...
        for category, adr, item in self.columns:
            ...
            if self.start_time and self.stop_time:
                data_for_graph = data_for_graph.loc[
                    (data_for_graph['time'] >= self.start_time) & (
                        data_for_graph['time'] <= self.stop_time)
                ]
        ...
        if flight_data and 'intervals' in flight_data:
            ...
            for interval in intervals:
                ...
                roi.setBrush(pg.mkBrush(192, 192, 50, 28))
